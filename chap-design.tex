...
% --- Step 8 (conditional) ---
\node[condbox, below=0.75cm of s7] (cond) {AILS\\pipeline?};

\node[stepbox, fill=violet!8, below right=0.6cm and 1.5cm of cond] (s8) {\textbf{Step 8:} Integral Image Computation};
\node[note, right=0.3cm of s8] {Summed-area table; $O(|V|)$};

% --- Step 9 ---
\node[endbox, below=1.8cm of cond] (s9) {\textbf{Step 9:} Output};
\node[note, right=0.3cm of s9] {Grid + start/goal + integral image};
...
\draw[arrow] (cond.east) -- ++(0.5cm,0) |- node[pos=0.25, above, font=\scriptsize] {Yes} (s8.west);
\draw[arrow] (s8.south) |- (s9.east);
...
\textbf{4-Connected Grid (Drone Implementation).} For the drone-based planning pipeline (Chapter~\ref{chap:implementation}), each non-boundary vertex had up to four cardinal neighbours, and all edge costs were uniform:
...
\begin{tikzpicture}[
    ...
    decision/.style={diamond, draw, thick, fill=yellow!10, aspect=2.8, font=\small, align=center,
                     inner sep=2pt},
    stratbox/.style={rectangle, draw, thick, fill=#1, minimum width=4.8cm, minimum height=1.4cm,
...
\node[stratbox=green!10, right=2.8cm of d1] (base) {\textbf{Strategy 1: Base}\\[2pt]\scriptsize Fixed-width corridor, radius $r_{\min}$\\[1pt]\scriptsize No per-point density computation};

% --- Decision 2 ---
\node[decision, below=1.2cm of d1] (d2) {$\max_{p} |\nabla\sigma(p)|$\\$< 0.1$?};

% --- Standard Strategy (right of d2) ---
\node[stratbox=cyan!10, right=2.8cm of d2] (standard) {\textbf{Strategy 2: Standard}\\[2pt]\scriptsize Density-adaptive radius via Eq.~\eqref{eq:radius_standard}\\[1pt]\scriptsize $r(p) = r_{\min} + \lfloor (r_{\max} - r_{\min}) \cdot \sigma(p)^{\alpha} \rfloor$};

% --- Predictive Strategy ---
\node[stratbox=violet!10, below=1.2cm of d2] (predictive) {\textbf{Strategy 3: Predictive}\\[2pt]\scriptsize Gradient-enhanced radius via Eq.~\eqref{eq:radius_gradient}\\[1pt]\scriptsize $r(p) = r_{\min} + \lfloor (r_{\max} - r_{\min}) \cdot (\sigma(p) + \beta\,|\nabla\sigma(p)|)^{\alpha} \rfloor$};

% --- Build Corridor ---
\node[procbox, fill=orange!8, below=1.0cm of predictive] (build) {Build adaptive corridor $\mathcal{C}_a$ using selected strategy};
...
% Strategies to Build Corridor
\draw[arrow] (base.south) |- ([yshift=0.2cm]build.east);
\draw[arrow] (standard.south) |- (build.east);
...
% --- No path exists ---
\node[failbox, fill=gray!15, below=0.8cm of fallback] (nopath) {No path in $G$};
...
\draw[darrow] (fallback.west) -- ++(-1.0cm,0) |- node[pos=0.5, below, font=\scriptsize\itshape, text=red!60!black] {Expand corridor, retry search} ([yshift=-0.25cm]s4.south) -- (s4.south);

% --- Full grid check ---
\draw[arrow] (fallback.south) -- node[right, font=\scriptsize] {$\mathcal{C}_a = V$} (nopath.north);
...
These defaults were adopted from the AILS study \citep{Elshahed2025AILS}. In the AILS experiments, parameter sensitivity was assessed by systematically varying $r_{\min}$, $r_{\max}$, $\alpha$, and $\omega$ while holding other parameters at their defaults (see Chapter~\ref{chap:discussion}).
...
\textbf{Random Seed Policy.} \textbf{TODO: Confirm the random seed policy used for reproducibility---whether a fixed global seed was set or per-map seeds were recorded.}
...
\textbf{TODO: Confirm whether the drone-based experiments (Pipeline~4) used a separate hardware configuration and, if so, specify it here or in Chapter~\ref{chap:implementation}.}
...
Chapter~\ref{chap:implementation} describes the software implementation of the framework, including the Python codebase, the drone integration via DroneKit and ROS/MAVROS, and the experimental execution environment.
