...
The research framework comprised three methodological pipelines, each designed to address a specific aspect of the investigation. Together, the pipelines formed a progressive study: from establishing classical baselines, through corridor-constrained search, to adaptive corridor construction. Figure~\ref{fig:framework_overview} illustrates the overall framework and the relationships among the three pipelines.
...
% --- Three Pipelines ---
\node[pipebox=red!8, below=1.4cm of preproc.south west, xshift=1.5cm] (p1) {};
...
\node[pipebox=cyan!8, right=0.7cm of p1] (p2) {};
...
\node[pipebox=violet!8, right=0.7cm of p2] (p3) {};
...
% --- Evaluation Metrics ---
\node[metricbox, below=0.9cm of p2.south] (metrics) {Evaluation Metrics};
...
\caption{Overview of the proposed research framework. The three pipelines share common grid representations and evaluation metrics but differ in their search-space restriction strategy.}
\label{fig:framework_overview}
\end{figure}

The three pipelines were defined as follows:

\begin{enumerate}[label=\textbf{Pipeline~\arabic*:}, leftmargin=3.5cm]

\item \textbf{Baseline Planner Pipeline.} This pipeline executed classical pathfinding algorithms---A*, Dijkstra's algorithm, Breadth-First Search (BFS), Depth-First Search (DFS), and Greedy Best-First Search---on the full grid without any search-space restriction. Its purpose was to establish performance baselines against which the corridor-based methods could be compared. The pipeline is detailed in Section~\ref{sec:meth_baseline_pipeline}.
...
All three pipelines shared the same grid representation, preprocessing procedure, and evaluation metrics, thereby ensuring that performance differences were attributable solely to the search strategy employed.
...
This section describes the four datasets used throughout the experimental evaluation and the preprocessing steps applied to turn raw environments into binary occupancy grids.
...
\item \textbf{Grid Construction.} The binary image was mapped to an occupancy grid $G = (V, E, w)$, where each pixel corresponded to one vertex $v \in V$, and edges connected each vertex to its valid neighbours under the chosen connectivity model (8-connected for DS1--DS3; 4-connected where applicable).
...
This section formalises the algorithmic building blocks that all three planning pipelines share. Each component is defined independently so that later sections can reference it without repetition.
...
\textbf{4-Connected Grid.} In a 4-connected configuration, each non-boundary vertex had up to four cardinal neighbours, and all edge costs were uniform:
\begin{equation}\label{eq:cost_4conn}
w(v, v') = 1 \quad \text{for all cardinal moves}.
\end{equation}

The 4-connected model simplifies waypoint generation, as cardinal-only movement translates directly to axis-aligned segments.
...
This section describes how the system chose which corridor construction method to apply for a given query, along with the fallback expansion procedure that ensured completeness.
...
The goal of the ILS pipeline was to measure what happens to classical pathfinding performance when the search space is restricted to a corridor. Each algorithm's neighbour expansion was confined to the ILS corridor, and the resulting changes in execution time, visited nodes, and path length were measured against the unconstrained baselines.
...
The baseline pipeline ran each classical pathfinding algorithm on the full grid without any search-space restriction, thereby establishing the unconstrained performance envelope for every algorithm across all datasets.
...
The AILS pipeline set out to determine whether a variable-width corridor---one whose radius at each reference-line cell adapts to the local obstacle density---could deliver robust search-space reduction across diverse obstacle topologies, including environments where a uniform-width corridor would either waste space in open regions or prove too narrow near obstacles.
...
These defaults were adopted from the AILS study \citep{Elshahed2025AILS}. In the experiments, parameter sensitivity was assessed by systematically varying $r_{\min}$, $r_{\max}$, $\alpha$, and $\omega$ while holding other parameters at their defaults; results of this ablation study are presented in the next chapter.
...
The experimental protocol was designed with reproducibility and statistical rigour in mind:
...
This chapter presented the methodology adopted to investigate corridor-based pathfinding on grid maps. The research framework comprised three pipelines---Baseline, ILS, and AILS---sharing a common grid representation, preprocessing procedure, and evaluation protocol.
