...
For grid-based pathfinding, Dijkstra's algorithm supports heterogeneous traversal costs, enabling cost maps where different terrain types impose different movement penalties. This capability is essential for risk-aware pathfinding where exposure costs must be accumulated along paths.
...
Field D* \citep{Ferguson2006} extends D* Lite to produce any-angle paths by interpolating costs across cell boundaries. Rather than restricting paths to grid edges, Field D* allows the robot to cut corners and take direct paths when beneficial.
